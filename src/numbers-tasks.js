...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const part1 = Math.acos(x1 * x2 + y1 * y2);
  const part2 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const part3 = Math.sqrt(x2 ** 2 + y2 ** 2);
...
function getCountOfOddNumbers(number) {
  const num = Math.abs(Math.trunc(number / 2));
  return number % 2 === 0 ? num : num + 1;
}
